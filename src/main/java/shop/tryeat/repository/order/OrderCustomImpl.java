...
import static shop.tryeat.domain.member.entity.QMember.member;
import static shop.tryeat.domain.order.entity.QOrder.order;
...
import shop.tryeat.domain.order.dto.QOrderSearchDto;
...
public class OrderCustomImpl implements OrderCustom {
    ...
    private List<OrderSearchDto> orderSearchContents(Member findMember) {
        ...
        List<OrderSearchDto> content = queryFactory.select(qOrderSearchDto).from(order).where(order.member.eq(findMember)).orderBy(order.createDate.desc()).join(order.member, member).fetch();
    ...
    public JPAQuery<Long> totalCount(Member member) {
        return queryFactory.select(order.count()).from(order).where(order.member.eq(member));
    }

    private Order findById(Long orderId) {
        return queryFactory.select(order).from(order).where(order.id.eq(orderId)).fetchOne();
    }
